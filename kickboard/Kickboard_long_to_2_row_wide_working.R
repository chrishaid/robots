...
print("Loading Kickboard data . . . ")
in.file<-paste("culture-analysis_",format(todays.date,"%y%m%d"),".csv", sep="")
kb.long<-read.csv(in.file)


#Discriminate between Create Points and Paycheck Decutions
print("Separating Create Points and paycheck deductions . . . ")
kb.long$CreatePoint<-is.na(kb.long$Dollar.Value)


#Remove any deposit.
print("Remove depostis . . . ")
...
#write the file
print("Writing file . . . ")
out.file<-paste("KB_",format(todays.date,"%y%m%d"),".csv",sep="")
write.csv(z,out.file, row.names=FALSE)

print(paste("Kickboard Data saved to: ",out.file,sep=""))
